...
local function processLocalizationData(data)
  local result = {}
  for k, v in pairs(data) do
    if v == true then
      result[k] = k
    elseif type(v) == "table" then
      result[k] = processLocalizationData(v)
    else
      result[k] = v
    end
  end
...
local L = setmetatable(
  processLocalizationData(localizationData[locale] or localizationData["enUS"]),
  {
    __index = function(tab, key)
      rawset(tab, key, tostring(key))
      return tostring(key)
    end
  }
)
